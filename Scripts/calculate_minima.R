...
chela <- read.csv(paste0(data_dir, "specimen_chela.csv")) %>% # already != HT 17, only shell 2, no special projects
  filter(SPECIES == "SNOW") %>%
...
write.csv(min.dat, "./Output/opilio_cutline_minima_arithmeticCW.csv")
...
ggsave("./Figures/density_plots_arithmeticCW.png", width = 11, height = 8.5)
...
labs <- data.frame(x = 50, y = 3, lab = paste0("R-squared = ", round(summary(mod)$r.squared, 2), "\np < 0.001"))
...
ggsave("./Figures/cutline_lm_plot_arithmeticCW.png", width = 6, height = 5)
